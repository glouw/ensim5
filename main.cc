...
struct sdl_s
{
    ...
    const double port_w_p = chamber_w_p / 4.0;
    const double port_h_p = chamber_w_p / 4.0;
    ...
    void draw_chambers(const std::vector<std::vector<double>>& open_ratios)
    {
        ...
        for(int x = 0; x < engine_w; x++)
        {
            ...
            port.y = chamber.y + chamber.h - port_h_p / 2.0;
            port.w = port_w_p;
            port.h = port_h_p;
    ...
    void draw_plots(const ensim::diags& diags)
    {
        ...
        for(int y = 0; y < diags.channels; y++)
        {
            ...
            const int points = data.size();
            const int max_points = plot_w_p;
            const int point_count = std::min(points, max_points);
            set_color(green);
    ...
    void draw_pressure_volume(const ensim::diags& diags)
    {
        if(pressure_volume_window)
        {
            ...
            SDL_FRect outline;
            outline.x = pv_window_x_p - 1;
            outline.y = pv_window_y_p - 1;
            outline.w = pv_window_w_p + 2;
            outline.h = pv_window_h_p + 2;
            set_color(white);
            ...
            const int size = static_pressure_pa.size();
            ...
            const double margin_x_p = rect.w * 0.05;
            const double margin_y_p = rect.h * 0.05;
